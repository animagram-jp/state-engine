...
impl ParsedManifest {
    ...
    /// Parses a YAML string into the manifest structures.
    /// `filename` is interned as the root node (index 1) of the key list.
    ///
    /// # Examples
    ///
    /// ```
    /// use state_engine::common::parser::ParsedManifest;
    ...
    /// pm.parse("cache", yaml).unwrap();
    ///
    /// // index 1 is the filename root record
    /// let root = pm.keys.get(1).unwrap();
    /// let dyn_idx = bit::get(root, bit::OFFSET_DYNAMIC, bit::MASK_DYNAMIC) as u16;
    /// assert_eq!(pm.dynamic.get(dyn_idx), Some("cache"));
    /// ```
    pub fn parse(&mut self, filename: &str, yaml: &str) -> Result<(), String> {
        ...
        self.keys.set(file_idx, file_record);

        Ok(())
    }

    /// Traverses a field key node (non-meta key).
    /// `ancestors` は filename を除いたfield keyのpath segments（qualify用）
    fn traverse_field_key(
        ...
        let dyn_idx = self.dynamic.intern(key_str);
        ...
        let key_idx = self.keys.push(record);

        // build current path for children
        ...
        if let Value::Mapping(mapping) = value {
            ...
            for (k, v) in mapping {
                ...
                if k_str.starts_with('_') {
                    let meta_idx = self.traverse_meta_key(k_str, v, filename, &current)?;
                    meta_indices.push(meta_idx);
                } else {
                    let child_idx = self.traverse_field_key(k_str, v, filename, &current)?;
                    child_indices.push(child_idx);
                }
            }
    ...
    fn traverse_meta_key(
        ...
        if let Value::Mapping(mapping) = value {
            ...
            for (k, v) in mapping {
                let k_str = yaml_str(k)?;
                let child_idx = self.traverse_prop_key(k_str, v, filename, ancestors)?;
                child_indices.push(child_idx);
            }
    ...
    fn traverse_prop_key(
        ...
        let (prop_val, client_val) = match key_str {
            "client"     => (bit::PROP_NULL, self.parse_client(value)),
        ...
        if key_str == "type" {
            let type_val = self.parse_type(value);
        ...
        if key_str == "map" {
            if let Value::Mapping(mapping) = value {
                ...
                for (k, v) in mapping {
                    let k_str = yaml_str(k)?;
                    let child_idx = self.traverse_map_key(k_str, v, filename, ancestors)?;
                    child_indices.push(child_idx);
                }
                let record = self.keys.get(key_idx).unwrap();
                let record = match child_indices.len() {
    ...
    /// key_str は qualified path としてpath mapに格納する。
    fn traverse_map_key(
        &mut self,
        key_str: &str,
        value: &Value,
        ...
        // map key は owner path + key_str で qualify
        let qualified = build_qualified_path(filename, ancestors, key_str);
        let seg_indices: Vec<u16> = qualified.split('.')
            .map(|seg| self.dynamic.intern(seg))
            .collect();
        let path_idx = self.path_map.push(seg_indices);
        ...
        Ok(self.keys.push(record))
    }

    /// Builds a YAML value record ([u64; 2]) from a scalar or template string.
    /// path tokenはqualifyしてpath mapに格納する。
    /// qualify不能（file not found相当）の場合はエラーを出さず相対pathのまま格納する。
    fn build_yaml_value(
        &mut self,
        value: &Value,
        filename: &str,
        ...
        for (i, token) in tokens.iter().enumerate().take(6) {
            let dyn_idx = if token.is_path {
                ...
                let seg_indices: Vec<u16> = qualified.split('.')
                    .map(|seg| self.dynamic.intern(seg))
                    .collect();
                self.path_map.push(seg_indices)
            } else {
                self.dynamic.intern(&token.text)
            };
        ...
        Ok(self.values.push(vo))
    }

    fn parse_client(&self, value: &Value) -> u64 {
    ...
    fn parse_type(&self, value: &Value) -> u64 {
        let s = match value { Value::String(s) => s.as_str(), _ => "" };
        match s {
...
/// - `${connection.common}` → already absolute (contains '.') → そのまま
/// - `${org_id}` → relative → `filename.ancestors.org_id` に展開
/// - qualify不能の場合はそのまま返す（エラーなし）
fn qualify_path(path: &str, filename: &str, ancestors: &[&str]) -> String {
    // already contains a dot → treat as absolute
    if path.contains('.') {
        return path.to_string();
    }
    // relative: prepend filename + ancestors
...
";
    ...
    fn test_parse_session_yaml() {
        let mut pm = ParsedManifest::new();
        pm.parse("session", YAML_SESSION).unwrap();

        // "sso_user_id" is interned
        let idx = pm.dynamic.intern("sso_user_id");
        assert_ne!(idx, 0);

        // key list has more than null slot
        assert!(pm.keys.get(1).is_some());
    }
    ...
    fn test_field_key_record_root_is_null() {
        ...
        // index 1 = "sso_user_id" field key record
        let record = pm.keys.get(1).unwrap();
    ...
    fn test_meta_key_record_root_index() {
        ...
        // scan key list for _state record (ROOT_STATE)
        let mut found = false;
        for i in 1..=20 {
            if let Some(r) = pm.keys.get(i) {
    ...
    fn test_type_index_integer() {
        ...
        // scan for a record with TYPE_I64
        let mut found = false;
        for i in 1..=20 {
            if let Some(r) = pm.keys.get(i) {
    ...
    fn test_static_value_interned() {
        ...
        // "request-attributes-user-key" should be in dynamic pool
        let idx = pm.dynamic.intern("request-attributes-user-key");
        assert_ne!(idx, 0);
    }
    ...
    fn test_template_value_is_template_flag() {
        let mut pm = ParsedManifest::new();
        pm.parse("cache", YAML_CACHE).unwrap();

        // scan yaml values for is_template=1
        let mut found = false;
        for i in 1..=30 {
            if let Some(vo) = pm.values.get(i) {
    ...
    fn test_path_token_stored_in_path_map() {
        ...
        // path map should have at least one entry beyond null
        assert!(pm.path_map.get(1).is_some(), "path map is empty");
    }
    ...
    fn test_qualify_path_absolute() {
        // already absolute (contains dot) → unchanged
